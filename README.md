...
**Kernels — full PolyBench/C 4.2.1 (30/30) + extras:** single — `gemm`, `syrk`, `syr2k`, `syrk_tri`, `syr2k_tri`, `floydwarshall`; multi-statement — `2mm`, `3mm`, `mvt`, `atax`, `bicg`, `gesummv`, `gemver`, `covariance`, `correlation`, `doitgen` (3-D); imperfect-nest solvers/BLAS — `trisolv`, `lu`, `cholesky`, `ludcmp`, `durbin`, `gramschmidt`, `trmm`, `symm`, `nussinov`; stencils — `jacobi1d`, `jacobi2d`, `seidel2d`, `heat3d` (3-D), `fdtd2d`, `adi`, `deriche`. Spanning matmul, matvec, reduction, **triangular**, **fusion**, **in-place**, **datamining**, **3-D**, **time-stepped stencils** (jacobi parallelizes; seidel carries dependences → needs skewing, correctly rejected), and **loop-carried solvers** (sequential carried loops correctly rejected; independent foralls accepted + checksum-verified). Drive any of them: `python3 run_agent.py --kernel <name>`.
...
**full PolyBench/C 4.2.1 — 30/30 kernels × 5 size classes = 150 instances:**
- **30/30 kernels** across every PolyBench category — blas (gemm/gemver/gesummv/symm/syr2k/syrk/trmm), kernels (2mm/3mm/atax/bicg/doitgen/mvt), solvers (cholesky/durbin/gramschmidt/lu/ludcmp/trisolv), datamining (correlation/covariance), medley (deriche/floyd-warshall/nussinov), stencils (adi/fdtd-2d/heat-3d/jacobi-1d/jacobi-2d/seidel-2d) — wired into `optimize`/`check_legality` (and the TUI), each at **5 size classes** (`MINI/SMALL/MEDIUM/LARGE/EXTRALARGE`, pass `size=`). **Every computational pattern works**: matmul, matvec, reduction, triangular, multi-statement, fusion, in-place, datamining, time-stepped stencils, 3-D, and the loop-carried solvers.
- **Imperfect-nest codegen** (`compilot/imperfect.py`) handles statements at differing loop depths with sibling inner loops under a loop-carried outer loop (cholesky's per-row j-loop + diagonal k-loop + sqrt; ludcmp's two j-loops + descending back-solve; gramschmidt; durbin; trmm; symm; nussinov). A tree codegen walks the per-statement loop lists keeping the common prefix open. Legality uses one binding statement per kernel: it correctly **rejects** parallelism on the sequential carried loop (cholesky's `i`/`j`/`k`, lu's `k`, trisolv's `i`/`j`, nussinov's `i`/`j`, durbin/ludcmp/gramschmidt) and **accepts + checksum-verifies** it on the independent ones (lu's `i`/`j`, trmm's `j`, symm's `j`).
- **Backward sweeps** (adi's Thomas back-substitution, deriche's right-to-left/bottom-up IIR passes, ludcmp's back-solve) run over an ascending index `jj` with `j=N-2-jj`, so the forward-only emitters stay correct while the dependence direction is modeled faithfully.
- *ponytail caveat:* the legality + codegen infrastructure is the deliverable, not solver speedups — at PolyBench sizes the per-step-barrier parallel solvers don't beat serial `-O3`, so the search keeps the identity; and the single-binding-statement legality model is exact for these kernels but would need a union-domain model for a kernel whose binding dependence spans statements.
