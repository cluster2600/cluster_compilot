...
**Pending:**
- **full PolyBench/C 4.2.1 (150 instances)** — **22/30 kernels** × **5 size classes** (`MINI/SMALL/MEDIUM/LARGE/EXTRALARGE`, the ×5 instance dimension — done; pass `size=` to `optimize`/`check_legality` or `--size` to the TUI). **every computational pattern works** (matmul, matvec, reduction, triangular, multi-statement, fusion, in-place, **datamining** incl. correlation, time-stepped stencils, **3-D** incl. heat-3d, and **imperfect-nest solvers** incl. trisolv/lu). The remaining ~8 are the other loop-carried *solvers* (cholesky/ludcmp/durbin/gramschmidt), imperfect-nest BLAS (symm/trmm), and the harder stencils/medley (fdtd-2d/adi/nussinov/deriche). **Imperfect-nest codegen** (statements at differing loop depths + loop-carried outer loops) is now in (`compilot/imperfect.py`): the legality engine correctly rejects parallelism on the sequential carried loop (lu's `k`, trisolv's `i`/`j`) and accepts it on the independent ones (lu's `i`). Caveat: the single-work-share emitter parallelizes only the outermost loop enclosing all statements, so a legal-but-shallower forall (lu's `j`) is reported `unsupported` rather than mis-emitted; and at PolyBench sizes the per-step-barrier parallel solvers don't beat serial `-O3`, so the search keeps the identity — the deliverable here is the *legality + codegen infrastructure*, not a solver speedup
