...
**Pending:**
- **full PolyBench/C 4.2.1 (150 instances)** — **20/30 kernels** × **5 size classes** (`MINI/SMALL/MEDIUM/LARGE/EXTRALARGE`, the ×5 instance dimension — done; pass `size=` to `optimize`/`check_legality` or `--size` to the TUI). **every computational pattern works** (matmul, matvec, reduction, triangular, multi-statement, fusion, in-place, **datamining** incl. correlation, time-stepped stencils, **3-D** incl. heat-3d). The remaining ~10 are loop-carried *solvers* (cholesky/lu/ludcmp/trisolv/durbin/gramschmidt), imperfect-nest BLAS (symm/trmm), and the harder stencils/medley (fdtd-2d/adi/nussinov/deriche) — all need **imperfect-nest codegen** (statements at differing loop depths + loop-carried substitution)
